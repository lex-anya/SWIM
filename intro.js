...
function drawFigures(opacity = 0.10) {
  const figH = H * 0.13;
  const figures = [
    { x: W * 0.455 },
    { x: W * 0.48 },
...
function drawMuseum() {
  ...
  ceilingLight.addColorStop(0, 'rgba(255, 210, 130, 0.3)');
...
canvas.addEventListener('click', (e) => {
  // Check if clicked on any glass case
  cases.forEach(c => {
    const w = BASE_W * c.depth;
    const h = BASE_H * c.depth;
    const left = c.cx - w / 2;
